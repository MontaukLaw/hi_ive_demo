    ...
    HI_S32 SAMPLE_INNER_CODEC_CfgAudio(AUDIO_SAMPLE_RATE_E enSample)
    {
        ...
        if (1) /* should be 1 when mic in */
        {
            ...
            *******************************************************************************************/
            ...
            // ACODEC_SET_ADCL_VOL 左声道音量
            // AACODEC_GET_ADCL_VOL：获取左声道输入的音量控制。
            if (ioctl(fdAcodec, ACODEC_GET_INPUT_VOL, &iAcodecInputVol))
            {
                printf("%s:ACODEC_GET_INPUT_VOL failed\n", __FUNCTION__);
                return HI_FAILURE;
            }
            ...
            // {
            //     printf("%s: ACODEC_SET_GAIN_MICL failed\n", __FUNCTION__);
            //     printf("ioctl err!\n");
            // }

            printf("get acodec micin volume: %d\n", iAcodecInputVol);
        }
    ...
    void *SAMPLE_COMM_AUDIO_AencProc(void *parg)
    {
        ...
        while (pstAencCtl->bStart)
        {
            ...
            if (FD_ISSET(AencFd, &read_fds))
            {
                ...
                // 获取时间戳
                printf("audio create time per: %lld us, len:::: %d\n", getCurrentTimeUs() - elapse_us, stStream.u32Len);
